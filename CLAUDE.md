...
| Boundary | Why frozen |
|---|---|
| **V11 statistical gate** (`MIN_TRADES_FOR_SIG`, PF 95% CI, PSR/DSR thresholds — DSR ≥ 0.90 (operator methodology decision 2026-06-15; was 0.95), walk-forward stability cutoffs) + **V60 economic gate** (`ANNUALIZED_RETURN_PASS_THRESHOLD_PCT=10.0` on annualized geometric_return_pct_at_alloc_90, 365-day year) | Methodology contract. Loosening to fit a candidate is fraud. The 2026-06-15 DSR drop to 0.90 was an explicit, deliberate operator methodology decision applied platform-wide — not a fit to any pending candidate. +20bps slippage check was retired — slippage_haircut_pnl is still logged for audit but no longer gates the verdict. |
